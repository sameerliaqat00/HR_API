...
namespace HR_API.Controllers
{
    ...
    public class LoanEligibleMonthController : ControllerBase
    {
        ...
        public async Task<ActionResult<APIResponse>> GetLoanEligibleMonths([FromQuery] string? search, int pageSize = 0, int pageNumber = 1)
        {
            try
            {
                IEnumerable<LoanEligibleMonth> loanEligibleList;
                loanEligibleList = await _repository.GetAllAsync(pageSize: pageSize,
                        pageNumber: pageNumber);

                if (!string.IsNullOrEmpty(search))
                {
                    loanEligibleList = loanEligibleList.Where(u => u.LoanEligibleMonths.ToLower().Contains(search));
                }
                Pagination pagination = new() { PageNumber = pageNumber, PageSize = pageSize };

                Response.Headers.Add("X-Pagination", JsonSerializer.Serialize(pagination));
                _response.Result = _mapper.Map<List<CityDTO>>(loanEligibleList);
        ...
        [HttpPut("{id:int}", Name = "UpdateLoanEligibleMonth")]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
